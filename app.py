...
st.title("Ordenador de Programa de Mantenimiento")
st.write(
    "Sube tu Excel original (.xls o .xlsx) y la app generara un nuevo archivo agrupado por "
    "matricula (AC) y WO, con WEEKLY CHECK, DAILY CHECK, EO normales y defectos al final."
)
...
with st.expander("Opciones avanzadas"):
    incluir_nr = st.checkbox(
        ...
        value=False,
        help="Dejalo desactivado si quieres omitir defectos N/R sin EO.",
    )
    ordenar = st.selectbox(
        "Orden de grupos AC/WO",
        options=["ac_wo", "aparicion"],
        format_func=lambda x: "Por AC y WO" if x == "ac_wo" else "Mantener orden de aparicion",
    )
